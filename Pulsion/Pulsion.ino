...
*/
struct pot_t {
  uint8_t lastValues[NUM_FILTER_SAMPLES];
  uint8_t channel;
  uint8_t control;
...
void updateAnalogValues() {
  ...
  if (i == 32) {
    ...
    for(int k = 0; k<NUM_FILTER_SAMPLES; k++){
      average += analogInputs[i].lastValues[k];
    }

    average = average >> 2;

    if ((average - actualValue) > 1 || (average - actualValue) < -1) {
      controlChange(8, 14, 127 - average);
